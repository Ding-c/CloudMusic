...
axios.all([MvComment(),MvAddress()])
.then(axios.spread(function(commentData,addressData){
    console.log(addressData.data.data.name) //歌曲名
    console.log(addressData.data.data.artistName) //歌手名
    console.log(addressData.data.data.brs['240']) //画质
    // console.log(commentData.data.hotComments[0].user.nickname) //评论者
    // console.log(commentData.data.hotComments[0].user.avatarUrl) //评论头像
    // console.log(commentData.data.hotComments[0].time) //评论时间
    // console.log(commentData.data.hotComments[0].content) //评论内容
    // console.log(commentData.data.hotComments[0].likedCount)//点赞数


    let songName = document.querySelector(".songName")
    let songArt = document.querySelector(".songArt")
    let mv = document.querySelector("video")
    
    songName.innerHTML = addressData.data.data.name
    songArt.innerHTML = "By:" + addressData.data.data.artistName
    mv.setAttribute("src",addressData.data.data.brs['240'])

    //热门评论
    let hotStr = ``
    commentData.data.comments.forEach((value,index,arr)=> {
        let avatarUrl=``
        let nickname = ``
        let repliedStr =``
        let res = ``
    ...
    `
    document.querySelector(".mvComment>ul").innerHTML=hotStr
})
